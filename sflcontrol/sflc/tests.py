...
class UserTest:
  ...
  def get_transactions_by_account(self):
    ...
    try:
      if self.status:
        response = requests.request("GET", url = f"http://127.0.0.1:8000/transaction?acc_id={self.acc_ids[0]}", headers={}, data={})
        if response.status_code == 200:
          response = json.loads(response.text)
...
def transaction_crud():
  
  user_test = UserTest(usr_doc = 1234567890, usr_pwd = "TestPassword")
  print(f"{bcolors.OKGREEN}Begin User Transaction Crud!{bcolors.ENDC}")
  user_test.create_user()
  user_test.login_user()
  ...
  user_test.create_transaction()
  user_test.get_transactions_by_account()
  user_test.get_transaction()
  user_test.update_transaction()
  user_test.delete_transaction()
